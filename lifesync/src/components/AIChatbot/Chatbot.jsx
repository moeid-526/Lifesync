...
import { FaCamera, FaUpload, FaImage, FaTimes, FaAngleLeft, FaAngleRight } from "react-icons/fa";
...
const Chatbot = () => {
  ...
  const fetchChatById = async (chatId) => {
    try {
      ...
      if (data) {
        ...
        setSidebarActive(false); // Close sidebar when selecting a chat
      }
    } catch (error) {
  ...
  const startNewChat = async () => {
    ...
    setSidebarActive(false); // Close sidebar when starting new chat
  ...
  return (
    <div className="dashboard-layout-chatbot">
      ...
      <div
        className={`sidebar-toggle ${sidebarActive ? 'open' : 'closed'}`}
        onClick={() => setSidebarActive(!sidebarActive)}
      >
        {sidebarActive ? <FaAngleLeft size={20} /> : <FaAngleRight size={20} />}
      </div>

      {/* Sidebar - Always in DOM but controlled by CSS */}
      <motion.div
