...
const Chatbot = () => {
  ...
  const getKerasResponse = async (inputText) => {
    try {
      ...
      return "Sorry, I'm having trouble connecting to the Keras model.";
    }
  };
    ...
    try {
      ...
      if (responseFound) {
        ...
        if (selectedModel === "llama") {
          ...
          [_, data] = await Promise.all([minLoadingTime, backendFetch]);
        } else if (selectedModel === "keras") {
          const kerasResponse = await getKerasResponse(input);
          data = { response: kerasResponse };
        } else if (selectedModel === "gemini") {
          const geminiResponse = await getGeminiResponse(input);
          data = { response: geminiResponse };
        }
